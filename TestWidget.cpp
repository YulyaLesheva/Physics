...
void TestWidget::Update(float dt)
{
	...
	for (int i = 0; i < AllBodies.size(); ++i) {
		for (int j = i; j < AllBodies.size(); ++j) {
			...
			if (result.colliding) {
				// мб нужно
		/*		if (!a->IsAwake()) {
					if (b->GetKineticForce() > 2 * a->GetSleepEpsilon()) {
						a->SetAwake(true);
					}
				}
				if (!b->IsAwake()) {
					if (a->GetKineticForce() > 2 * b->GetSleepEpsilon()) {
						b->SetAwake(true);
					}
				}*/
	...
	//if (!_DarkBlueBody->IsAwake()) Log::Info("dark blue sleep");
	//else {
	//	Log::Info("dark blue not sleep");
	//}
	//
	//if (!_GreenLine->IsAwake()) Log::Info("green line sleep");
	//else {
	//	Log::Info("green line not sleep");
	//}
